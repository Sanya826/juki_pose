...
class RandomRotateTrans(Preprocess):
    def __init__(self, max_rotate_degree=40, max_trans=10):
        super().__init__()
        self.log = logging.getLogger(self.__class__.__name__)
    ...
    def __call__(self, image, anns, meta):
        ...
        trans = ((dice_x - 0.5) * 2 * self.max_trans, (dice_y - 0.5) * 2 * self.max_trans)
        img_rot, R = self.rotate_bound(np.asarray(image), np.copy(degree), np.copy(trans), (128, 128, 128))
        ...
        for j,ann in enumerate(anns):
            for k in range(12):
                ...
                bool_w = new_xy[0] < w
                bool_h = new_xy[1] < h
                if (not bool_a.all()) or (not bool_w) or (not bool_h):
                    anns[j]['keypoints'][k,:2] = [0,0]
                    anns[j]['keypoints'][k, 2] = 0
    ...
    def rotate_bound(self, image, angle, trans, bordervalue):
