...
from lib.network.rtpose_vgg import use_vgg, get_model
from lib.datasets import coco, transforms, datasets
from lib.config import update_config
from lib.utils.logger import Logger
from evaluate.JUKI_eval import run_eval

DATA_DIR = './data/coco'

ANNOTATIONS_TRAIN = [os.path.join(DATA_DIR, 'annotations', item) for item in ['JUKI_train.json']]
ANNOTATIONS_VAL = os.path.join(DATA_DIR, 'annotations', 'JUKI_val.json')
IMAGE_DIR_TRAIN = os.path.join(DATA_DIR, 'images/JUKI_train')
IMAGE_DIR_VAL = os.path.join(DATA_DIR, 'images/JUKI_val')
...
def train_cli(parser):
    ...
    group.add_argument('--batch-size', default=2, type=int,
                       help='batch size')
...
def evaluate(model):
    with torch.autograd.no_grad():
        ...
        weight_name = "./network/weight/best_pose_optuna.pth"
...
def run_train(trial):
    ...
    deg = trial.suggest_int("Rotate Degree", 10, 60)
    trans = trial.suggest_int("Translate", 10, 50)
    ...
    preprocess = transforms.Compose([
            ...
            transforms.RandomRotateTrans(max_rotate_degree=deg, max_trans=trans),
        ])
    ...
    #logとweightのディレクトリを作成
    if not os.path.exists("./logs"):
        print("Make log directory")
        os.mkdir("logs")
    if not os.path.exists("./network/weight"):
        print("Make Weight Directory")
        os.makedirs("./network/weight")

    #loggerをセット
    writer = Logger(log_dir="./logs/openpose_org")
    #logを格納する場所の用意
    ...
    optimizer_1 = trial.suggest_categorical("Optimizer stage1", ["Adam", "AdamW", "RMSprop"])
    print("optimizer:", optimizer_1)
    if optimizer_1 == "Adam":
        lr = trial.suggest_loguniform("lr stage1:", 1e-5, 1e-3)
        beta1 = trial.suggest_uniform("beta1 stage1", 0.89, 0.99)
        beta2 = trial.suggest_uniform("beta2 stage1", 0.99, 0.9999)
        eps = trial.suggest_loguniform("epsilon stage1", 1e-13, 1e-8)
        optimizer = torch.optim.Adam(trainable_vars, lr=lr, betas=(beta1, beta2), eps=eps)
    elif optimizer_1 == "AdamW":
        lr = trial.suggest_loguniform("lr stage1:", 1e-5, 1e-3)
        beta1 = trial.suggest_uniform("beta1 stage1", 0.89, 0.99)
        beta2 = trial.suggest_uniform("beta2 stage1", 0.99, 0.9999)
        eps = trial.suggest_loguniform("epsilon stage1", 1e-13, 1e-8)
        optimizer = torch.optim.AdamW(trainable_vars, lr=lr, betas=(beta1, beta2), eps=eps)
    elif optimizer_1 == "RMSprop":
        lr = trial.suggest_loguniform("lr stage1:", 1e-5, 1e-4)
        momentum = trial.suggest_uniform("momentum stage1", 0.0, 1.0)
        optimizer = torch.optim.RMSprop(trainable_vars, lr=lr, momentum=momentum)
    ...
    for epoch in range(5):
        ...
        log_dict['train/train_loss'] = [train_loss, epoch]
        log_dict['val/val_loss'] = [val_loss, epoch]
        writer.add_log(log_dict)
    ...
    optimizer_2 = trial.suggest_categorical("Optimizer stage2", ["Adam", "AdamW", "RMSprop"])
    if optimizer_2 == "Adam":
        lr = trial.suggest_loguniform("lr stage2:", 1e-5, 1e-3)
        beta1 = trial.suggest_uniform("beta1 stage2", 0.89, 0.99)
        beta2 = trial.suggest_uniform("beta2 stage2", 0.99, 0.9999)
        eps = trial.suggest_loguniform("epsilon stage2", 1e-13, 1e-8)
        optimizer = torch.optim.Adam(trainable_vars, lr=lr, betas=(beta1, beta2), eps=eps)
    elif optimizer_2 == "AdamW":
        lr = trial.suggest_loguniform("lr stage2:", 1e-5, 1e-3)
        beta1 = trial.suggest_uniform("beta1 stage2", 0.89, 0.99)
        beta2 = trial.suggest_uniform("beta2 stage2", 0.99, 0.9999)
        eps = trial.suggest_loguniform("epsilon stage2", 1e-13, 1e-8)
        optimizer = torch.optim.AdamW(trainable_vars, lr=lr, betas=(beta1, beta2), eps=eps)
    elif optimizer_2 == "RMSprop":
        lr = trial.suggest_loguniform("lr stage2:", 1e-5, 1e-4)
        momentum = trial.suggest_uniform("momentum stage2", 0.0, 1.0)
        optimizer = torch.optim.RMSprop(trainable_vars, lr=lr, momentum=momentum)
    ...
    model_save_filename = './network/weight/best_pose_optuna.pth'
    for epoch in range(5, args.epochs):
        ...
        if is_best:
            torch.save(model.module.state_dict(), model_save_filename)
    writer.close()
...
if __name__=="__main__":
    ...
    study.optimize(run_train, n_trials=100)
