...
def kp_connections(keypoints):
    kp_lines = [
        [keypoints.index('left_finger_tip'), keypoints.index('left_finger_base')],  
        [keypoints.index('left_finger_base'), keypoints.index('wrist')],
        [keypoints.index('right_finger_tip'), keypoints.index('right_finger_base')],
        [keypoints.index('right_finger_base'), keypoints.index('wrist')],                
        [keypoints.index('wrist'), keypoints.index('elbow')],
        [keypoints.index('elbow'), keypoints.index('shoulder')],
        [keypoints.index('shoulder'), keypoints.index('body')],          
        [keypoints.index('body'), keypoints.index('left_knee')],
        [keypoints.index('body'), keypoints.index('right_knee')],     
        [keypoints.index('left_knee'), keypoints.index('left_foot')],        
        [keypoints.index('right_knee'), keypoints.index('right_foot')],    
    ]
    return kp_lines
...
class CocoKeypoints(torch.utils.data.Dataset):
    ...
    def __init__(self, root, annFile, image_transform=None, target_transforms=None,
        ...
        self.cat_ids = self.coco.getCatIds(catIds=[2])
        if all_images:
    ...
    def get_ground_truth(self, anns, neck_bool=False):
        ...
        keypoints = []
        for ann in anns:
            single_keypoints = np.array(ann['keypoints']).reshape(12,3)
            if neck_bool:
