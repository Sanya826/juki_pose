...
def build_names(stage_num, trunk="paf"):
    names = []

    for j in range(1, stage_num+1):
            names.append('loss_%s_%d' % (trunk, j))
    return names


def get_loss(saved_for_loss, heat_temp, vec_temp, stage_num_info):
    '''
    stage_num_info[0]: paf_stageの数
    stage_num_info[1]: cm_stageの数
    '''

    paf_names = build_names(stage_num_info[0], trunk="paf")
    cm_names = build_names(stage_num_info[1], trunk="cm")
    saved_for_log = OrderedDict()
    criterion = nn.MSELoss(reduction='mean').cuda()
    total_loss = 0

    for j in range(stage_num_info[0]):
        pred1 = saved_for_loss[j]
        loss1 = criterion(pred1, vec_temp)
        total_loss += loss1
        saved_for_log[paf_names[j]] = loss1.item()

    for j in range(stage_num_info[1]):  
        pred2 = saved_for_loss[j+stage_num_info[0]]
        loss2 = criterion(pred2, heat_temp) 
        total_loss += loss2
        saved_for_log[cm_names[j]] = loss2.item()
...
def train(train_loader, model, optimizer, epoch, stage_num_info):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    
    meter_dict = {}
    paf_name = build_names(stage_num_info[0], trunk="paf")
    cm_name = build_names(stage_num_info[1], trunk="cm")
    names = paf_name + cm_name
    for name in names:
    ...
    for i, (img, heatmap_target, paf_target) in enumerate(train_loader):
        ...
        total_loss, saved_for_log = get_loss(saved_for_loss, heatmap_target, paf_target, stage_num_info)
...
def validate(val_loader, model, epoch, stage_num_info):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
...
if __name__ == "__main__":
    ...
    paf_stage_num = 4
    cm_stage_num = 3
    stage_num_info = (paf_stage_num, cm_stage_num)

    # model
    model = get_model(trunk='vgg19', paf_stage_num=paf_stage_num, cm_stage_num=cm_stage_num)
    ...
    #loggerをセット
    writer = Logger(log_dir="./logs/openpose_imp")
    #logを格納する場所の用意
    ...
    for epoch in range(5):
        # train for one epoch
        train_loss = train(train_loader, model, optimizer, epoch, stage_num_info)

        # evaluate on validation set
        val_loss = validate(val_loader, model, epoch, stage_num_info)  
    ...
    model_save_filename = './network/weight/best_mypose.pth'
